...
def evaluate_predictions(preds: List[List[float]],
    ...
    :param ale_unc: A list of lists of shape :code:`(data_size, num_tasks)` with aleatoric uncertainties.
    ...
    if model.aleatoric:
        valid_ale_unc = [[] for _ in range(num_tasks)]
    ...
    for i in range(num_tasks):
        for j in range(len(preds)):
            if targets[j][i] is not None:  # Skip those without targets
                ...
                if model.aleatoric:
                    valid_ale_unc[i].append(ale_unc[j][i])
    ...
    for i in range(num_tasks):
        ...
        for metric, metric_func in metric_to_func.items():
            ...
            # elif model.aleatoric:
            #     results[metric].append(metric_func(valid_targets[i], valid_preds[i], valid_ale_unc[i]))
            else:
                results[metric].append(metric_func(valid_targets[i], valid_preds[i]))
